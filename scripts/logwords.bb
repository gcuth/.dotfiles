  ...
  (let [{:keys [dir log n]} (cli/parse-opts *command-line-args* cli-opts)]
    (cond (nil? dir) (println "Please provide a directory to count words in.")
          ...
          :else (do (log-total-word-count dir log)
                    ...
                    (let [lowest-wc (apply min (read-log log))
                          highest-wc (apply max (read-log log))
                          max-wc-diff (- highest-wc lowest-wc)
                          max-wc-diff (if (pos? (int max-wc-diff))
                                        (str "+" max-wc-diff)
                                        (str max-wc-diff))
                          deltas-now (->> (read-log log)
                                          (calculate-deltas))
                          ewma-now (int (ewma deltas-now))
                          max-now (int (apply max deltas-now))]
                      (println (str (int (avg [ewma-now max-now]))
                                    " (" max-wc-diff ")")))))))
